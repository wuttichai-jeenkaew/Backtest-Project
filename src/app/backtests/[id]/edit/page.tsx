...
export default async function EditBacktestPage({ params }: EditBacktestPageProps) {
  const { id } = await params;
  const [backtest, systems] = await Promise.all([getBacktest(id), getSystems()]);
  ...
  return (
    <>
      <Header title={`Edit: ${backtest.tradingSystem.name} - ${backtest.symbol}`} />
      <div className="flex flex-1 flex-col gap-4 p-4 max-w-3xl">
        <BacktestForm
          ...
          initialData={{
            ...
            grossProfit: backtest.grossProfit ? Number(backtest.grossProfit) : null,
            grossLoss: backtest.grossLoss ? Number(backtest.grossLoss) : null,
            maxDrawdown: backtest.maxDrawdown ? Number(backtest.maxDrawdown) : null,
            ...
            averageWin: backtest.averageWin ? Number(backtest.averageWin) : null,
            averageLoss: backtest.averageLoss ? Number(backtest.averageLoss) : null,
            largestWin: backtest.largestWin ? Number(backtest.largestWin) : null,
            largestLoss: backtest.largestLoss ? Number(backtest.largestLoss) : null,
